...
export default function AdJobApproved() {
    ...
    const getAllApprove = async () => {
        let data = await adminService.jobAllApprove();
        setData(data.data);
    };
    useEffect(() => {
        getAllApprove();
    ...
    return (
        <AdminLayout>
            <>
                {data.length > 0 ? (
                    <>
                        <h2 className="text-2xl font-semibold pb-4">Việc làm đã duyệt</h2>
                        <div className="px-4">
                    ...
                    <h2 className="text-2xl font-semibold pb-4">Không có việc đã duyệt</h2>
                )}
            </>
        </AdminLayout>
    );
